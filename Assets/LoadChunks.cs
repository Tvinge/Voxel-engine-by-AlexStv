...
public class LoadChunks : MonoBehaviour
{
    ...
    static WorldPos[] chunkPositions = //queue of chunk positions radiating from 0.0.0 sorted by coseness to the centre 
    ...
    void FindChunksToLoad()
    {
        ...
        //if there aren't already chunks to genereate 
        if (updateList.Count == 0)
        {
            //cycle through the array of positions
            for (int i = 0; i < chunkPositions.Length; i++)
            {
                //translate the player position and array position into chunk postion
                ...
                if (newChunk != null && (newChunk.rendered || updateList.Contains(newChunkPos)))
                    continue;
                //load a column of chunks in this postion
                for (int y = -4; y < 4; y++)
                {
    ...
    void LoadAndRenderChunks()
    {
        ...
        {//loops through 8 chunks to be built and builds them
            for (int i = 0; i < buildList.Count && i < 8; i++)
            {
                BuildChunk(buildList[0]);
                buildList.RemoveAt(0);
            }
             //You can limit how intensive this is per frame by changing the for loop around the build list and the call to BuildChunk to iterate more or fewer times per frame.
            //If chunks were built return early
            return;
        }
        if (updateList.Count != 0)
        {//loops throught all chunks to update and updates them
